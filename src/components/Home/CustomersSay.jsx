...
function CustomersSay() {
    ...
    useEffect(() => {
        const fetchTestimonials = async () => {
            try {
                const res = await axios.get('https://ayuras.life/api/v1/testimonials');
    ...
    if (testimonials.length === 0) return <div className="text-center py-10">No testimonials available.</div>;
            ...
            <div className="relative max-w-xl mx-auto mt-8 overflow-hidden">
                <div className="p-6 bg-white rounded-xl shadow-md text-center">
                    ...
                    <div className="flex items-center justify-center mt-4">
                        ...
                        <div className={current.image ? "ml-3 text-left" : "text-left"}>
                            <h4 className="font-semibold">{current.name}</h4>
                            {current.location && <p className="text-sm text-gray-500">{current.location}</p>}
                        </div>
                    </div>
                </div>
